...
async function sendCompleted(){
    ...
    const Score_data = {local_class, local_Id, completed: true, version:1};
    const options ={
        method: 'POST',
        headers:{
...
function initialiseChart(){
    const data_chart = {
        labels: [
          'Distinguer tri, rangement et classement',
          'Utilisation de la clé de détermination',
          "Réaliser un tri",
          'La classification phylogénétique',
          'Observation des êtres vivants et de leur caractéristiques'
        ],
        datasets: [{
          label: 'Vos compétences',
          data: [donnéesGraph[0], donnéesGraph[1], donnéesGraph[2], donnéesGraph[3], donnéesGraph[4],],
          fill: true,
