...
function modify_href(){
    var ref = "/download?name="+input_class.value+"&sheet=0";
...
function switch_to_prof(){
  if(!partie_prof){
    ...
    document.getElementById("greetings").textContent ="Bienvenue à ce test diagnostique sur le signe moins";
  ...
  if(!server_data_statut){
    ...
    if( server_data_statut){
      ...
      document.getElementById("class_check").style.display = "none";
        ...
        document.getElementById("canvas").style.top = '5rem';
...
imgtemp.setAttribute("id","titreQuestions" );
imgtemp.src ="signe moins/questions/images/titre2.PNG";
imgtemp.style.width = WIDTH + "px";
imgtemp.style.marginBottom = "-5px";
document.getElementById("canvas").insertBefore(imgtemp, document.getElementById("canv"));
...
async function send_Id_class(){
    ...
    const data_id_class = {local_class,local_Id, version: 0};
    const options ={
        method: 'POST',
        headers:{
