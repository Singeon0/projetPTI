...
//recyclage de "connect the dots" pour le test sur le signe moins 
...
const MARGIN = HEIGHT - (GRID_SIZE+1)*CELL+50; // top margin for score...
...
canv.width = WIDTH;
    ...
    data_question = q;
    data_numb = n;
    NOMBRE2 = Math.min(NOMBRE_QUESTION, NOMBRE_REPONSE);
    NOMBRE = Math.max(NOMBRE_QUESTION, NOMBRE_REPONSE);
...
function drawConsigne_relier(){
    var temp_image = document.getElementById("consignes_connect1_img");
    ctx.drawImage(temp_image, 85, 20, 800, 70);
}
     ...
     OUTER:  for(let i =0; i < 2; i++){
      ...
      //il commence par repérer dans quelle case est le curseur
           ...
           //si un carré est déja sélectionné, que le carré sur lequel on vient de cliquer est différent du précédent et qu'il n'est pas déjà utilisé alors trace une ligne depuis le carré déjà séléctionné jusqu'à celui que l'on vient de cliquer
                 if(select == true && selected < 4 && i != temp_i && squares_connect_the_dots[i][j] != squares_connect_the_dots[temp_i][temp_j] && squares_connect_the_dots[i][j].used != true){
...
function getGridY_connect_the_dots(row, N){
 ...
 function highlightDot(i, j){
     ctx.fillStyle = "green";
     ctx.beginPath();
     ctx.arc(squares_connect_the_dots[i][j].x ,squares_connect_the_dots[i][j].y, DOT, 0, Math.PI*2);
     ctx.fill();
 }
