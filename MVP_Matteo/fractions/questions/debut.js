...
function modify_href(){
    var ref = "/download?name="+input_class.value+"&sheet=2";
...
function switch_to_prof(){
  if(!partie_prof){
    ...
    document.getElementById("greetings").textContent ="Bienvenue à ce test diagnostique sur les fractions";
  ...
  if(!server_data_statut){
    ...
    if( server_data_statut){
      ...
      document.getElementById("class_check").style.display = "none";
        ...
        document.getElementById("canvas").style.top = '8rem';
        ...
        if(server_data_passer > 14){
          temp.textContent = "Partie 2 "
        }else{
          temp.textContent = "Partie 1 "
        }
        document.getElementById("body").appendChild(temp);
        var int1 = setInterval(()=>{debutQuestion();clearInterval(int1);}, 1500);
      }
}
function debutQuestion(){
...
async function send_Id_class(){
    ...
    const data_id_class = {local_class,local_Id, version: 2};
    const options ={
        method: 'POST',
        headers:{
